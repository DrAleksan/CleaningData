...
* "id" column received from "subject_test(_train).txt" file
* "activity" clomn received from "y_test(_train).txt" file, wtih changing numbers to activities names from "activity_labels.txt" file
* All other columns were received from X_test.txt and X_train.txt, by selecting only columns with std and mean values, and then culculating mean of each column for each "id" and "activity"

## Variables type

* "id" and "activity" is a factor
* All other columns is numeric
